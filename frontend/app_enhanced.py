        ...
        if intel_resp is not None and intel_resp.status_code == 200:
            ...
            with ic2:
                ...
                st.caption(
                    ...
                    f"(requires >=2 modalities with score > 0.4)"
                )
                st.caption(
        ...
        with st.expander("⚙️ Advanced Overrides (Fusion Strategy & Primary Dataset)", expanded=False):
            ...
            with _fusion_col:
                _chosen_fusion = st.selectbox(
                    ...
                    options=fusion_opts,
                    help="Forces a specific fusion method. Leave as 'concatenation' to let the system choose.",
...
def render_phase_3_preprocessing():
    ...
    with st.expander("🧠 Feature Intelligence (semantic roles, signals, predictability)", expanded=False):
        ...
        if st.session_state.get("session_id"):
            ...
            if _fi:
                ...
                # Per-modality predictability
                _pred = _ctx.get("modality_predictability", {}) or {}
                if _pred:
                    st.markdown("**Per-modality predictability (RandomForest 3-fold CV)**")
                    _df_pred = pd.DataFrame([
                        {"modality": k, "predictability": v}
                        for k, v in _pred.items()
                    ])
                    st.dataframe(_df_pred, width="stretch")
    ...
    # ---- Image Preprocessing (dynamic — reflects what configure() chose) ----
    ...
    if image_sample or output_shapes.get("image"):
        with st.expander("View Image Preprocessing Details", expanded=True):
            ...
            _missing = (image_sample or {}).get("missing_paths", 0)

            before_col, after_col = st.columns(2)
            with before_col:
                st.markdown("**Before (raw)**")
                st.caption("Pixels loaded per batch during training — not during preprocessing")
                st.code(f'df["{_col_name}"] = "img/42953.png"')
                if _missing:
                    st.warning(f"{_missing}/10 sample paths not found on disk")

            with after_col:
                st.markdown("**After (transform pipeline)**")
                _steps = []
                if _gray:
                    _steps.append("Grayscale(1 channel)")
                _steps.append(f"Resize({_tgt[0]}×{_tgt[1]})")
                _steps.append(f"Augmentation: {_aug}")
                if _sharp:
                    _steps.append("Sharpening (low-quality detected)")
                _steps.append("ToTensor()")
                _steps.append("Normalize(ImageNet μ/σ)")
                for _step in _steps:
                    st.markdown(f"→ `{_step}`")

            _channels = 1 if _gray else 3
            st.caption(f"**Output shape:** `(N, {_channels}, {_tgt[0]}, {_tgt[1]})`  |  "
                       f"Augmentation intensity: **{_aug}**"
                       + (" | Grayscale" if _gray else "")
                       + (" | Sharpening enabled" if _sharp else ""))
    ...
    **Selection Criteria:**
    ...
    if st.session_state.model_selected:
        ...
        if eligible_modalities:
            st.write(f"**Eligible Modalities:** {', '.join(str(m) for m in eligible_modalities)}")
        if excluded_modalities:
            with st.expander("Excluded Modalities", expanded=False):
                ex_rows = [
        ...
        with st.expander("\U0001f9e0 Why did AutoVision choose this model?", expanded=True):
            ...
            _n_mods = len(eligible_modalities) if eligible_modalities else 1
            ...
            if isinstance(_probe, (int, float)):
                ...
                _why_reasons.append(
                    f"**Heuristic ranking**: No probe was run (preprocessing may be "
                    f"needed first). `{_name}` was selected based on rule-based ranking."
                )
        ...
        if isinstance(probe_score, (int, float)):
            ...
            st.caption(
                "⚠️ No probe score - preprocessing must be run before model selection "
                "for data-driven ranking."
            )
        ...
        if fusion_probe:
            with st.expander("Fusion Strategy Probe", expanded=False):
                st.write(f"- Selected strategy: {fusion_probe.get('selected_strategy', 'N/A')}")
                st.write(f"- Method: {fusion_probe.get('method', 'N/A')}")
                scores = fusion_probe.get("scores", {}) if isinstance(fusion_probe, dict) else {}
                if scores:
                    score_df = pd.DataFrame(
                        [{"Strategy": k, "Score": v} for k, v in scores.items()]
                    )
                    st.dataframe(score_df.sort_values("Score", ascending=False), width="stretch")
                else:
        ...
        if jit_dry_run:
            with st.expander("JIT Encoder Dry-Run Results", expanded=False):
                ...
                if rationale:
                    _kv_table(rationale, "Selection Rationale")
                rc1, rc2 = st.columns(2)
                rc1.metric(
                    "Budget (MB)",
                    int(jit_dry_run.get("vram_budget_bytes", 0) / 1024 / 1024),
                )
                rc2.metric(
                    "Peak Used (MB)",
        ...
        if meta_context:
            with st.expander("Meta-Learning Context", expanded=False):
                st.caption(
                    "Historical similar runs used as bias only; they do not override the selected model."
                )
        ...
        if hpo:
            with st.expander("HPO Search Space"):
                _kv_table(hpo, "HPO Configuration")
        ...
        with st.expander("View Optuna Auto-Tuning Search Space"):
            if hpo_space:
                ...
                st.markdown(table)
                _kv_table(hpo_space, "HPO Search Space")
            else:
        ...
        if use_manual:
            ...
            with ac1:
                ...
                fusion_descriptions = {
                    "concatenation": "Direct concat. Fastest and stable for single modality.",
                    "attention": "Learns modality importance. Strong default for 2 modalities.",
                    "graph": "Explicit interaction graph across modalities.",
                    "uncertainty": "Inverse-variance weighting when modalities disagree.",
                    "uncertainty_graph": "Hybrid graph + uncertainty for 3+ modalities.",
                }
                fusion_idx = (
...
def render_phase_5_training():
    ...
    # ----- Failed -----
        ...
        error = task.get("error", "Unknown error")
        _show_error_with_retry(f"Training failed: {error}", "retry_training")
        st.session_state.phase_states[5] = {
            "status": "failed",
            "reason": str(error),
        }
...
def render_phase_6_monitoring():
    ...
    if not st.session_state.get("training_result") and not st.session_state.get("model_selected"):
