...
def select_fusion_strategy(schema_info: dict) -> str:
    ...
    if modset == {"image", "text"}:
        # High alignment (≥0.7) → modalities share semantics → StructuralSemantic
        # Low alignment (<0.4) → complementary signals → use Complementarity
        if alignment < 0.4:
            return "complementarity"
        return "structural_semantic"   # [1] for vision+language (default)
