...
class ApexLightningModule(pl.LightningModule):
    ...
    def __init__(
        ...
        self._binary_pos_weight: Optional[torch.Tensor] = None
        self._class_weights: Optional[torch.Tensor] = class_weights
        ...
        if problem_type == "classification_binary":
            ...
            if self._use_focal_loss:
                self.loss_fn = FocalLoss(gamma=self._focal_gamma, alpha=class_weights)
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        # Accumulate per-epoch alignment/contrastive loss for frontend ULA charts
        _align_val = self._metric_to_float(callback_metrics.get("alignment_mean_cosine"))
        _contrast_val = self._metric_to_float(callback_metrics.get("train/contrastive_loss"))
        if _align_val is not None:
            self._alignment_loss_history.append(_align_val)
        if _contrast_val is not None:
            self._contrastive_loss_history.append(_contrast_val)
    ...
    def _apply_adaptive_loss(self, base_loss: torch.Tensor) -> torch.Tensor:
        ...
        if self.contrastive_weight > 0.0 and self._last_encoded_batch:
            try:
