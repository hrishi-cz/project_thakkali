...
def _update_session_context_with_retry(
    ...
    Falls through without raising so callers treat the write as best-effort.
    """
...
async def select_model(request: Request) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "artifact_versions":  _contract["artifact_versions"],
            "eligible_modalities": (
                recommendations[0].get("eligible_modalities", modalities)
                if recommendations
                else modalities
            ),
            "excluded_modalities": (
    ...
    try:
        ...
        if _training_ctx is not None:
            try:
                ...
                ContextValidator.require_fusion_consistency(
                    fusion_strategy=str(
                        getattr(_training_ctx, "fusion_strategy", "concatenation")
                        or "concatenation"
                    ),
                    modalities=modalities,
                    phase="/train-pipeline preflight",
        ...
        async def _run_training() -> None:
            try:
                ...
                # Update ExecutionContext with training results
                try:
                    if _sid:
                        ctx = session_manager.get_session(_sid)
                        if ctx:
                            ctx.update_training(phase5)
                            ctx.active_modalities = list(modalities)
                            ctx.training_signals = {
...
async def monitor_overview(request: Request) -> Dict[str, Any]:
    ...
    try:
        ...
        monitor_payload = DriftAdapter().build_monitor_payload(drift_data)
...
def _run_inference_task(
    ...
    try:
        ...
        payload = {
            ...
            "calibration_applied": _cal is not None,
            "calibration_method":  getattr(_cal, "method", None) if _cal else None,
            "context_stage": _contract["context_stage"],
            "context_version": _contract["context_version"],
            "artifact_versions": _contract["artifact_versions"],
        }
...
async def get_drift_status(session_id: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "drift_details": dict(getattr(ctx, "drift_details", {}) or {}),
    }
