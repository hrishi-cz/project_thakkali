...
class GlobalAggregationService:
    ...
    def infer_global_schema(
        ...
        # Calculate confidence with safety check for empty sequence
        column_counts = [len(p.schema_result.get('columns', {})) for p in profiles if p.schema_result]
        if column_counts:
            confidence = len(common_columns) / max(column_counts)
        else:
            confidence = 0.0
